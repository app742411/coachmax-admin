...
const SectionHeader: React.FC<SectionHeaderProps> = ({ icon: Icon, title }) => (
  <div className="flex items-center gap-2 mb-4 pb-2 border-b border-gray-100 dark:border-gray-800">
    ...
    <h5 className="text-lg font-bold text-gray-800 dark:text-gray-100 uppercase tracking-tighter italic">{title}</h5>
  </div>
);
        ...
        {steps.map((step) => (
          <div key={step.id} className="flex items-center gap-3 shrink-0">
            ...
            <div className="hidden sm:block">
              <p className={`text-[10px] font-black uppercase tracking-widest ${currentStep === step.id ? "text-gray-900 dark:text-white" : "text-gray-400"}`}>Step 0{step.id}</p>
              <h6 className={`text-xs font-bold uppercase italic tracking-tighter ${currentStep === step.id ? "text-brand-500" : "text-gray-400"}`}>{step.label}</h6>
            </div>
          </div>
        ))}
      </div>
        ...
        <div className="flex items-center justify-between pt-8 px-2">
          {currentStep > 1 && (
            <Button
              ...
              variant="outline"
              onClick={prevStep}
              className="rounded-xl px-8 font-black uppercase italic tracking-widest border-gray-200 text-gray-500 flex items-center gap-2"
            >
          ...
          <div className="ml-auto flex items-center gap-4">
            {currentStep < 3 ? (
              <Button
                type="button"
                onClick={nextStep}
                className="rounded-xl px-12 font-black uppercase italic tracking-widest shadow-xl shadow-brand-500/20 flex items-center gap-2"
              >
                Next Step
              ...
              <Button
                ...
                disabled={loading}
                className="rounded-xl px-16 font-black uppercase italic tracking-widest shadow-xl shadow-brand-500/30 bg-success-600 hover:bg-success-700 active:scale-95 transition-all text-white"
              >
