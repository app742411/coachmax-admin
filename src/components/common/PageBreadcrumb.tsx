        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 rounded-xl border border-gray-100 bg-white p-5 dark:border-gray-800 dark:bg-gray-900 shadow-sm lg:p-6 transition-all">
            ...
            <nav>
                <ol className="flex items-center gap-2">
                    <li>
                        <Link
                            className="inline-flex items-center gap-2 text-sm font-bold  text-gray-400 hover:text-brand-500 transition-colors"
                            to="/"
                        >
                            Home
                    ...
                    {items.map((item, index) => (
                        <li key={index} className="flex items-center gap-2">
                            {item.path ? (
                                ...
                                <span className="text-xs font-bold  text-brand-600 underline decoration-2 underline-offset-4">
                                    {item.name}
                                </span>
                            )}
                        </li>
                    ...
                    {(!items || items.length === 0) && (
                        <li className="text-sm font-bold  text-brand-600 underline decoration-2 underline-offset-4">
                            {pageTitle}
                        </li>
                    )}
                </ol>
