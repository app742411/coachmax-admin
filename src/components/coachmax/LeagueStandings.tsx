  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6 shadow-sm h-full">
      <div className="flex items-center justify-between mb-6">
        ...
        <button className="text-brand-500 font-bold uppercase text-xs hover:underline decoration-2 underline-offset-4">Full Table</button>
      </div>
      ...
      <div className="overflow-x-auto">
        <Table>
          <TableHeader className="border-b border-gray-100 dark:border-gray-800">
            <TableRow>
              <TableCell isHeader className="py-3 px-2 text-[10px] font-black uppercase text-gray-400">Pos</TableCell>
              <TableCell isHeader className="py-3 text-[10px] font-black uppercase text-gray-400">Team</TableCell>
              <TableCell isHeader className="py-3 text-[10px] font-black uppercase text-gray-400 text-center">P</TableCell>
              <TableCell isHeader className="py-3 text-[10px] font-black uppercase text-gray-400 text-center">Pts</TableCell>
              <TableCell isHeader className="py-3 text-[10px] font-black uppercase text-gray-400">Form</TableCell>
            </TableRow>
          </TableHeader>
          <TableBody className="divide-y divide-gray-50 dark:divide-gray-800/50">
            {standings.map((team) => (
              <TableRow key={team.rank} className="group hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">
                <TableCell className="py-4 px-2 font-black text-gray-400 group-hover:text-brand-500">{team.rank}</TableCell>
                ...
                <TableCell className="py-4 text-center font-black text-brand-600">{team.points}</TableCell>
                <TableCell className="py-4">
                  <div className="flex gap-1">
                    {team.form.map((res, i) => (
                      <span key={i} className={`w-5 h-5 flex items-center justify-center rounded text-[9px] font-black text-white ${res === "W" ? "bg-green-500" : res === "D" ? "bg-gray-400" : "bg-red-500"
                        }`}>
                        {res}
                      </span>
                    ))}
