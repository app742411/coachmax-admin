      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
           ...
           <div className="flex items-center gap-3">
              <Link 
                to="/add-product" 
                className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-brand-500 text-white font-black italic shadow-xl shadow-brand-500/20 active:scale-95 transition-all text-sm uppercase"
              >
           ...
           {filteredProducts.map(product => (
             <div key={product.id} className="group bg-white dark:bg-gray-900 rounded-3xl border border-gray-100 dark:border-gray-800 overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-300 relative">
                ...
                {product.isOutOfStock && (
                   <div className="absolute top-4 left-4 z-10 bg-red-500 text-white px-3 py-1 rounded-full text-[10px] font-black uppercase italic shadow-lg">
                      Out of Stock
                   </div>
                )}
                ...
                <div className="p-6">
                   <div className="flex items-center gap-2 mb-3">
                      <span className="text-[10px] font-black uppercase text-brand-500 tracking-widest">{product.category}</span>
                   </div>
                   <h4 className="text-xl font-black italic tracking-wide text-gray-900 dark:text-white mb-2 line-clamp-1 truncate">{product.name}</h4>
                   <p className="text-gray-400 text-xs font-medium mb-4 line-clamp-2 min-h-[32px]">{product.subtitle}</p>
                   
                   <div className="flex items-center justify-between pt-4 border-t border-gray-50 dark:border-gray-800">
                      <div>
                         <span className="text-xs font-black text-gray-400 uppercase tracking-widest block mb-0.5">Starting at</span>
                         <span className="text-xl font-black text-brand-600 block italic tracking-tighter">${product.price} AUD</span>
                      </div>
        ...
        {filteredProducts.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 text-center">
             ...
             <h3 className="text-xl font-black italic uppercase text-gray-900 dark:text-white">Gear Not Found</h3>
